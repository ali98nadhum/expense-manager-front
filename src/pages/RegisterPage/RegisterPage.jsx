...
import successfulAne from "../../animation/Registrationsuccessful.json";
import { useState } from "react";

const schema = Yup.object().shape({
  email: Yup.string().email("يرجى ادخال اميل صالح").required("الاميل مطلوب"),
  password: Yup.string()
    .min(8, "الرمز يجب أن يكون 8 أحرف على الأقل")
    .required("الرمز مطلوب"),
  name: Yup.string()
    .required("يرجى ادخال الاسم")
    .min(3, "الاسم يجب ان يكون من 3 احرف على الاقل"),
});

const RegisterPage = () => {
  const { isLoading, setIsLoading, registerUser } = authStore();
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 flex items-center justify-center p-4">
        {isSuccess ? (
          <div className="flex items-center justify-center flex-col">
            <Lottie
              animationData={successfulAne}
              loop={false}
              className="h-[300px]"
            />
            <p className="text-white text-2xl">عمله التسجيل تمت بنجاح</p>
          </div>
        ) : (
          <motion.div
